# https://nix.dev/tutorials/towards-reproducibility-pinning-nixpkgs.html#pinning-packages-with-urls-inside-a-nix-expression
# https://discourse.nixos.org/t/what-is-the-point-of-using-nix-shell-with-a-function/8473/1

# The commit hash is taken from status.nixos.org - the
# one  below  is  the  latest  `nixos-20.03`  channel.
# (Calling  `nix-shell` with  that  as `azure-cli`  is
# broken in the master branch.
...
# or use <nixpkgs>, hoping that the channel set up points to a stable nixpkgs revision
...
# https://discourse.nixos.org/t/use-buildinputs-or-nativebuildinputs-for-nix-shell/8464/2
  buildInputs = with pkgs; [
    azure-cli
    bash
    # Not  sure why  `cacert`  is needed  but here's  some
    # info:
  ...
  # Where  is it  documented that  environment variables
  # can be set just like this with `mkShell`?
